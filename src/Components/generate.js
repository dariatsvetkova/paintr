...
function generate(userColours) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (userColours.length > 0) {
      ...
      cols.forEach(col => {
        ...
        if (contrastWhite > 1 / 1.8) {
          ...
          if (col[2] >= 95) {
            light = white.length === 0 ? light : (white[2] > col[2] ? col : white);    // If there is already a white, compare their lightness
            ...
            accent1 = primary.length === 0 ? accent1 : (primary[1] < col[1] ? col : primary);    // The colour with more saturation is set as an accent
            primary = primary.length === 0 ? col : (primary[1] < col[1] ? primary : col);
        ...
        // In all other cases, use the user's colours as primary and accent:
          ...
          accent1 = primary.length === 0 ? accent1 : (primary[1] < col[1] ? col : primary);
          primary = primary.length === 0 ? col : (primary[1] < col[1] ? primary : col);
    ...
    primary = primary.length > 0 ? primary : [
      hue.length > 0 ? hue[Math.round(Math.random() * (hue.length - 1))] : Math.round(Math.random() * 360),  // Use the hue from user's colours or generate randomly
    ...
    switch (method) {
      ...
      case 'mono':
        int = (Math.round(Math.random()) * 2 - 1) * Math.round(Math.random() * 15);   // Random -15 to +15 deg shift from the hue to allow for more contrast
        
        accent1 = accent1.length > 0 ? accent1 : [
          primary[0] + int < 360 ? primary[0] + int : primary[0] + int - 360,
      ...
      case 'analogue':
        ...
        accent1 = accent1.length > 0 ? accent1 : [
          primary[0] + int < 360 ? primary[0] + int : primary[0] + int - 360,
          primary[1] > 50 ? primary[1] * (1 - Math.random()*0.15) : primary[1] / (1 - Math.random()*0.15),
          primary[2] > 50 ? primary[2] * (1 - Math.random()*0.25) : primary[2] / (1 - Math.random()*0.25)
        ];
        accent2 = [
      ...
      case 'comp':
        int = (Math.round(Math.random()) * 2 - 1) * Math.floor(Math.random() * (15 - 5 + 1)) + 5;   // Random +-5-15 deg shift from the opposing hue to avoid clashing colours

        accent1 = accent1.length > 0 ? accent1 : [
          primary[0] + 180 + int < 360 ? primary[0] + 180 + int : primary[0] + 180 + int - 360,
          primary[1] > 50 ? primary[1] * (1 - Math.random()*0.2) : primary[1] / (1 - Math.random()*0.2),
          primary[2] > 50 ? primary[2] * (1 - Math.random()*0.35) : primary[2] / (1 - Math.random()*0.35)
        ];
        accent2 = [
          accent1[0],
          primary[1],
      ...
      case 'split-comp':
        int = (Math.round(Math.random()) * 2 - 1) * Math.floor(Math.random() * (75 - 20 + 1)) + 20;   // Random 20-75 deg shift from the opposing hue
      ...
      case 'triad':
        int = (Math.round(Math.random()) * 2 - 1) * Math.floor(Math.random() * 20);   // Random -20 to +20 deg shift from each hue

        accent1 = accent1.length > 0 ? accent1 : [
          primary[0] + 120 + int < 360 ? primary[0] + 120 + int : primary[0] + 120 + int - 360,
          primary[1] > 50 ? primary[1] * (1 - Math.random()*0.1) : primary[1] / (1 - Math.random()*0.1),
          primary[2] > 50 ? primary[2] * (0.6 - Math.random()*0.2) : primary[2] / (0.6 - Math.random()*0.2)
        ];
        accent2 = [
          primary[0] - 120 - int > 0 ? primary[0] - 120 - int : primary[0] - 120 - int + 360,
          primary[1] > 50 ? primary[1] * (1 - Math.random()*0.15) : primary[1] / (1 - Math.random()*0.1),
          primary[2] > 50 ? primary[2] * (1 - Math.random()*0.25) : primary[2] / (1 - Math.random()*0.25)
        ];
        break;
      ...
      case 'square':
        ...
        accent1 = accent1.length > 0 ? accent1 : [
          primary[0] + 90 + int < 360 ? primary[0] + 90 + int : primary[0] + 90 + int - 360,
          primary[1] > 50 ? primary[1] * (1 - Math.random()*0.25) : primary[1] / (1 - Math.random()*0.25),
          primary[2] > 50 ? primary[2] * (1 - Math.random()*0.3) : primary[2] / (1 - Math.random()*0.3)
        ];
        accent2 = [
          primary[0] - 90 - int > 0 ? primary[0] - 90 - int : primary[0] - 90 - int + 360,
          primary[1] > 50 ? primary[1] * (1 - Math.random()*0.25) : primary[1] / (1 - Math.random()*0.25),
          primary[2] > 50 ? primary[2] * (1 - Math.random()*0.3) : primary[2] / (1 - Math.random()*0.3)
        ];
        break;
    ...
    const round = (array) => array.map((num, i) => { 
      num = i === 1 && num > 100 
        ? 100                               // Limit saturation to 100%
        : i === 2 && num > 100 ? 95 : num;  // Limit lightness to 95% (otherwise too close to white)
    ...
    let test = validateScheme(primary, accent1, accent2, white, light, dark, userColours);
...
function checkContrast(colour1, colour2) {
  ...
  return ((Math.min(lum1, lum2) + 0.05) / (Math.max(lum1, lum2) + 0.05)).toFixed(5);
}
...
function validateScheme(p, a1, a2, w, l, d, userColours) {
  ...
  if (checkContrast(w, d) > 1 / 7) {   // (has to match 7.0 : 1 ratio or higher)
  ...
  const check = (c1, c2) => {
    return checkContrast(c1, c2) <= 1 / 1.8 || (Math.abs(c1[0] - c2[0]) >= 35 && Math.abs(c1[0] - c2[0]) <= 325)
  }

  if (!check(p, a1) && !check(p, a2)) {   // Both accent colours are too close to primary
    console.log('both accents lack contrast with p');
    return false;

  } else if (check(p, a1) && !check(p, a2)) {   // Accent2 is too close to primary
    console.log('a2 lacks contrast with p');
    core.splice(2,1);    // Remove accent2 from the palette and only use accent1

  } else if (!check(p, a1) && check(p, a2)) {   // Accent1 is too close to primary
  ...
  } else {    // Both accent colours have enough contrast with primary
    if (!check(a1, a2)) {
      if (userColours.some(val => val.every((num, i) => num === a1[i]))) {
  ...
  core = core.map(col => {
    ...
    neutrals.forEach(neut => {
      checkContrast(col, neut) < 1 / 4.5 && colourObj.bckgrds.push(neut) && colourObj.foregrds.push(neut);
    });

    return colourObj;
  });

  if (!core.every((colourObj, i) => {
    if (colourObj.bckgrds.length > 0 && colourObj.foregrds.length > 0) {
      return true
    } else {
      if (i === 2) {
