...
import { IoShuffle, IoReturnDownBack, IoReturnUpForward, IoCopyOutline } from 'react-icons/io5';
  ...
  handleSubmit(event, input) {
    ...
    try {
      ...
      if (val.match(/^#[0-9A-F]{6}$/)) {
        val = hexToHsl(val);
        !cols.some(col => col.every((num, i) => num === val[i])) && cols.push(val);   // If the colour hasn't been added yet, add it to userColours
  ...
  render() {
    ...
    for (let key in colourSet) {
      if (Object.prototype.hasOwnProperty.call(colourSet, key)) {
        colourSet[key] = 
          (key === 'primary' || key === 'accent1' || key === 'accent2') && this.state[key].colour
          ? [hslToHex(this.state[key].colour), hslToHex(this.state[key].foregrds[0])]
          : this.state[key].length > 0 ? hslToHex(this.state[key]) : [];
      }
    }

    console.log('colourSet: ', colourSet);
    
    let UserColours = this.state.userColours.map((col, i) => {
      return (
        <UserColour 
          colour={col} 
          key={i}
    ...
    return (
      <section id="generate" className="generator">
        ...
        <div className="controls">
          ...
          {this.state.primary.colour && 
            <Palette colourSet={colourSet} />
          }

          <div className="buttons">
            <button onClick={this.mixup}><IoShuffle /></button>
            <button onClick={this.back}><IoReturnDownBack /></button>
            <button onClick={this.forward}><IoReturnUpForward /></button>
            <button onClick={this.copy}><IoCopyOutline /></button>
          </div>
        </div>
        
        <Mockup colourSet={colourSet} />
      </section>
    );
  }
}
